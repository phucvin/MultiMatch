...
function Board:performStartEffect()
	...
	local after = function ()
		...
		if LevelManager.instance():getLevelRatio() > 0 then
			tween(self, 0.5, {levelRatio = LevelManager.instance():getLevelRatio()})
		end
...
function Board:moveIn(after)
	self:seq(function ()
	tween(self, 1, {ox = self.ox}, {ox = self.ox + 600})
...
function Board:explode(depth)
	...
	for i = 1, self.nRows, 1 do
		for j = 1, self.nCols, 1 do
			...
			for k = j-1, 1, -1 do		  if gk[i][k] == gk[i][k+1] then ah = k else break end end
			for k = j+1, self.nCols, 1 do if gk[i][k] == gk[i][k-1] then zh = k else break end end
			for k = i-1, 1, -1 do		  if gk[k][j] == gk[k+1][j] then av = k else break end end
			for k = i+1, self.nRows, 1 do if gk[k][j] == gk[k-1][j] then zv = k else break end end
			...
			elseif zv-av+1 >= self.minGemsToExplode then
				for k = av, zv, 1 do
					gb[k][j] = true
				end
			end
	...
	for j = 1, self.nCols, 1 do
		...
		if n > 0 then
			totalNeedsToDone = totalNeedsToDone + n
			--explode gems
			...
			for k = 1, m, 1 do
				...
				stills[k].g:fallTo(self.ox + self.cellSize*(j-0.5), self.oy + self.cellSize*(self.nRows-k+0.5))
				end)
			end

			--explode gone gems and renew it
			for k = 1, n, 1 do
				gones[k].g:createExplode(function () nExploded = nExploded+1 end) --counting nExpolde
				--setup the gird to new gems
				--and make sure new gems dont make explode
				do
					local i = self.nRows-(m+k-1)
					--because we are on left->right, and bot->top
					local bot = -1
		    		local left = -1
		    		if i <= self.nRows - 1 then bot = gk[i+1][j] end
		    		if j > 1 then left = gk[i][j-1] end
		    		repeat gk[i][j] = math.random(1, self.nKinds) until gk[i][j] ~= bot and gk[i][j] ~= left
		    	end
				--gk[self.nRows-(m+k-1)][j] = math.random(1, self.nKinds)

				gg[self.nRows-(m+k-1)][j] = gones[k].g
				gones[k].g:hide() --hide if first, to show the explode
				self:seq(function ()
				while nExploded < n do yield() end --until all gems have exploded, we wait
				--then show the gems at the new position and fall it to the rigth pos too
				gones[k].g:show()
				gones[k].g:renew(gk[self.nRows-(m+k-1)][j], self.ox + self.cellSize*(j-0.5), self.oy - self.cellSize*(k-0.5))
				gones[k].g:fallTo(self.ox + self.cellSize*(j-0.5), self.oy + self.cellSize*(self.nRows-(m+k-1)-0.5), (k-1)*0.02,
	...
	if continue then
		...
		--add score
		LevelManager.instance():addScore(depth, totalNeedsToDone)
		self:seq(function ()
		tween(self, 1.5, {levelRatio = LevelManager.instance():getLevelRatio()})
		end)
		...
		self:seq(function ()
		tween(self, 0.5, {boardFactor = self.boardFactor + 0.1}, {})
		end)
...
function Board:endBoard(isWin, after)
	...
	if isWin then
		...
		tween(self, 1, {boardFactor = 0, boardShake = 0, levelRatio = 0}, {levelRatio = 1})
	else
